...
class JmImageTool:
    ...
    def save_image(cls, image: Image, filepath: str):
        ...
        try:
            ...
            try:
                ...
                jpg_path = Path(filepath).with_suffix(".jpg")
                image.save(str(jpg_path), format="JPEG")
    ...
    def open_image(cls, fp: Union[str, bytes]):
        ...
        try:
            ...
            # Android Chaquopy 环境下 Pillow 可能缺少 libwebp 支持，使用 Android 原生 BitmapFactory 兜底
            try:
                ...
                import logging
                logging.getLogger(__name__).warning("Android 原生 BitmapFactory 解码失败: %s", exc)
            raise
